...
def play():
    ...
    try:
        ...
        print(f"ERROR loading Actor: {e}"); env.close(); return
    ...
    for episode in range(NUM_EPISODES):
        ...
        while not done and not truncated:
            ...
            with torch.no_grad():
                # Use the preprocess_and_encode helper function
                z_t = preprocess_and_encode(obs, transform, vae_model, DEVICE)

                # 2. Get Action from Policy (Actor)
                dist = actor_model(z_t.unsqueeze(0)) # Add batch dimension

                # Choose deterministic action for evaluation/playback
                action_raw = dist.mean # Use mean for deterministic action
                # Or sample: action_raw = dist.sample()
